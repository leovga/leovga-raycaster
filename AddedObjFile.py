        ...
        for x in range(width):
            for y in range(height): # for each pixel
                ...
                for z in objlist:   # for each object
                    if isinstance(z, trimesh):
                        for a in z.trilist:
                            hit_data = a.intersect(shotray)
...
class trimesh(Surface):
        ...
        with open(fname, "r") as file:
            ...
            while line:
                ...
                if words[0] == "v":
                    ...
                    v.append(self.read_normal_data(words))
                elif words[0] == "vt":
                    v.append(self.read_texture_data(words))
                elif words[0] == "f":
                    self.read_face_data(words, v, vt, vn)
                line = file.readline()
        print("Done")
    ...
    def read_texture_data(self, words):
        return [
        float(words[1]),
        float(words[2])
        ]
    
    def read_face_data(self, words, v, vt, vn):
        points = []
        for item in words:
            if item == words[0]: print("Face")
            else:
                plist = []
                print("NUMBER:", int(item) - 1)
                for element in v[int(item) - 1]:
                    plist.append(element)
                points.append(point(float(plist[0]), float(plist[1]), float(plist[2])))
                print(points)
        self.trilist.append(triangle(points[0], points[1], points[2],
